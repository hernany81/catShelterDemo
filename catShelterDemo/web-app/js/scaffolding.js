 ...
 */
var scaffoldingModule = angular.module('scaffolding', ['grailsService', 'flashService', 'custom-widgets']);
...
function ListCtrl($scope, $routeParams, $location, Grails, Flash, MainAppService) {
    ...
    $scope.deleteConfirmed = function() {
    	$scope.itemToDelete.$delete(function(response) {
    ...
    MainAppService.title = $('body').data('list-title');
    MainAppService.action = 'list';
}

function ShowCtrl($scope, $routeParams, $location, Grails, Flash, MainAppService) {
    $scope.message = Flash.getMessage();
    ...
    MainAppService.title = $('body').data('show-title');
    MainAppService.action = 'show';
}

function CreateCtrl($scope, $location, Grails, Flash, MainAppService) {
    ...
    MainAppService.title = $('body').data('create-title');
    MainAppService.action = 'create';
}

function EditCtrl($scope, $routeParams, $location, Grails, Flash, MainAppService) {
    Grails.get({id: $routeParams.id}, function(item) {
        $scope.item = item;
    }, errorHandler.curry($scope, $location, Flash));
    ...
    MainAppService.title = $('body').data('edit-title');
    MainAppService.action = 'edit';
}

scaffoldingModule.service('MainAppService', function () {
	return {
		title: '', 
		action: '',
		selectedItem: null,
		getClassForAction: function(ac) {
			if(ac == this.action) {
				return 'active';
			}
			return '';
...
scaffoldingModule.controller('MainCrudCtrl', function MainCrudCtrl($scope, MainAppService) {
